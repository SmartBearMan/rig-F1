...
#include "compat/gmp-6.1.2/gmp.h"
#include "compat.h"
#include "miner.h"
...
extern bool opt_bitmain_sia_freq_ctrl; 
extern unsigned int opt_bitmain_sia_freq_array[BITMAIN_MAX_CHAIN_NUM][ASIC_NUM_EACH_CHAIN];
#endif

extern int last_temperature, temp_highest;
extern int opt_coin_type;
...
char displayed_av_hash_rate[16]={0};
char displayed_av_hash_rate2[16]={0};
...
void btc_regen_hash ( struct work* work )
{
	...
	data[19] = *(uint32_t*)(work->data+76) ;

	gen_hash (data,ohash,80);
}



void midstate_btc(uint32_t* midstate,uint32_t *pdata){
	uint32_t data[20];
	sha256_ctx ctx;
	
	be32enc_vect(data,pdata,19);
...
void gen_hash1(const unsigned char* data, unsigned int len, unsigned char* hash){
	unsigned char hash1[32];
...
void gen_hash2(const unsigned char* data, unsigned int len, unsigned char* hash){
	unsigned char hash1[32];
	sph_sha256_context ctx_sha2;
...
static char *set_coin_type(char *arg)
{
	 if(strcmp(arg, "sha256d")==0){
	    opt_coin_type = COIN_BTC;
	}else{
	    quit (1, "not support algorithm");
	}

	applog(LOG_NOTICE, "opt_coin_type %d", opt_coin_type);

	return NULL;
}

char *set_board0_freq(char *arg)
{
	//applog(LOG_NOTICE, "set_board0_freq %s", arg);

	const int chain = 0;
	int index = 0;
	int i = opt_bitmain_sia_freq;
	
	char* fq = strtok(arg,":");
	while(fq != NULL){
		if(set_int_0_to_9999(fq,&opt_bitmain_sia_freq_array[chain][index])!=NULL){
			opt_bitmain_sia_freq_array[chain][index] = opt_bitmain_sia_freq;
		}
		index ++;

		fq = strtok(NULL,":");

		if(index >= ASIC_NUM_EACH_CHAIN){
			break;
		}
	}
...
static struct opt_table opt_config_table[] =
{
...
#if  defined (USE_BITMAIN_D1) ||  defined(USE_BLACKMINER_F1_MINI)
    OPT_WITHOUT_ARG("--fan-ctrl",
                    opt_set_bool, &opt_bitmain_fan_ctrl,
                    "Enable bitmain miner fan controlling"),
    OPT_WITH_ARG("--fan-pwm",
    ...
    OPT_WITH_ARG("--bitmain-voltage",
                 ...
                 "Set voltage"),
    OPT_WITH_ARG("--coin-type",
                 set_coin_type, NULL, &opt_set_null,
                 "Set Mining Coin Type"),
    OPT_WITH_ARG("--vote",
                 set_int_1_to_65535,opt_show_intval, &opt_vote,
                 "Optional vote value for decred blocks"),
     OPT_WITHOUT_ARG("--freq-ctrl",
                    opt_set_bool, &opt_bitmain_sia_freq_ctrl,
                    "Enable freq controlling"),
     OPT_WITH_ARG("--board0-freq",
                    set_board0_freq, NULL,&opt_set_null,
                    "set board0 freq"),
     OPT_WITH_ARG("--board1-freq",
                    set_board1_freq, NULL,&opt_set_null,
                    "set board1 freq"),
     OPT_WITH_ARG("--board2-freq",
                    set_board2_freq, NULL,&opt_set_null,
                    "set board2 freq"),
#endif
...
{
	if(work->job_id!=NULL)  free(work->job_id);
    if(work->ntime!=NULL) free(work->ntime);
    if(work->coinbase!=NULL) free(work->coinbase);
    if(work->nonce1!=NULL) free(work->nonce1);
...
static bool stale_work(struct work *work, bool share)
{
    ...
    if (!share && pool->has_stratum)
    {
        ...
        if (pool->swork.job_id != NULL && strcmp(work->job_id, pool->swork.job_id))
            same_job = false;
        cg_runlock(&pool->data_lock);

        if (!same_job)
        {
            applog(LOG_NOTICE, "Work stale due to stratum job_id mismatch");
            return true;
        }
    }
...
static bool hash_push(struct work *work)
{
    ...
    /*applog(LOG_DEBUG, "hash_push push work job_id:%s, id:%d over.", work->job_id, work->id);*/
    mutex_unlock(stgd_lock);
...
{
    json_t *val = NULL,*val2=NULL, *err_val, *res_val, *id_val;
    ...
    int id;
   	...
   	applog(LOG_DEBUG,"parse_stratum_response: %s", s);
    ...
    if (!sshare)
    {
        ...
        {
            applog(LOG_ERR, "Accepted untracked stratum share from pool %d", pool->pool_no);
        ...
        {
            applog(LOG_ERR, "Rejected untracked stratum share from pool %d", pool->pool_no);
...
static json_t* http_curl(struct pool *pool,char* t_req)
{
    json_t *val;
    CURL *curl;
    char *tmp = pool->rpc_url+11;
    char tmp_url[50];

	//mutex_lock(&http_lock);

    strcpy(tmp_url,"http");
    strcpy(tmp_url+4,tmp);
    //applog(LOG_NOTICE, "tmp_url :%s",tmp_url);

    curl = curl_easy_init();
    if (unlikely(!curl))
        quit (1, "CURL initialisation failed in update_gbt");

    val = json_rpc_call2(curl, tmp_url, t_req, false, pool, true);

    /*if (val)
    {
    	applog(LOG_NOTICE, "SUCCEED json_rpc_call2");

		jsoin_id = json_object_get(val, "id");
		if (jsoin_id){
			 int x = json_integer_value(jsoin_id);
			 applog(LOG_NOTICE, "SUCCEED jsoin_id:%d",x);
		}

		result = json_object_get(val, "result");
		if (result){
			if(json_is_integer(result)){
				applog(LOG_NOTICE, "SUCCEED int result:%u",(uint32_t)json_integer_value(result));
			}else if(json_is_string(result)){
				applog(LOG_NOTICE, "SUCCEED result:%s",json_string_value(result));
			}
		}
    ...
    {
        applog(LOG_NOTICE, "FAILED json_rpc_call2");
    }*/
		
    curl_easy_cleanup(curl);
    //mutex_unlock(&http_lock);
...
void stratum_resumed(struct pool *pool)
{
    ...
    {
        applog(LOG_ERR, "Stratum connection to pool %d resumed", pool->pool_no);
        pool_resus(pool);
    }
}
...
static void *stratum_rthread(void *userdata)
{
    ...
    while (42)
    {
        ...
        fd_set rd;
        char *s;
		 struct nexues_packet packet;

        if (unlikely(pool->removed))
        {
        	 applog(LOG_DEBUG, "stratum rthread 1 to suspend_stratum");
	    ...
	    //applog(LOG_NOTICE, "rthread stratum recv!!!!!\n");
...
static void *stratum_sthread(void *userdata)
{
    ...
    while (42)
    {
        char noncehex[100], nonce2hex[80];
		 static char s[3000];
		 char votehex[16]={0};
        char * header;
        struct stratum_share *sshare;
	...
	uint64_t   * nonce2_64;
      ...
      if(opt_coin_type == COIN_BTC){
        ...
        *nonce2_64 = htole64(work->nonce2);
        ...
        if(opt_coin_type == COIN_BTC){
			...
			__bin2hex(nonce2hex,(const unsigned char *)nonce2_64,work->nonce2_len); 
	  }else{
            quit (1, "not support algorithm");
        }
	  ...
	  if(opt_coin_type == COIN_BTC){
			 snprintf(s, sizeof(s),
			        ...
			        applog(LOG_NOTICE, "submit : %s",s);

	 }else{
		quit (1, "not support algorith");
	}
        ...
        while (time(NULL) < sshare->sshare_time + 120)
        {
		 ...
		 int send_len = 0;
		 send_len = strlen(s);
            if (likely(stratum_send(pool, s, send_len)))
            {
            		mutex_lock(&sshare_lock);
...
static struct work *hash_pop(bool blocking)
{
    ...
    /*applog(LOG_DEBUG, "hash_pop  hc %d, work->job_id  %s, work->work_id %d", hc, work->job_id, work->id);*/
...
void set_target(unsigned char *dest_target, double diff)
{
    ...
    *data64 = htole64(h64);
    ...
    if (opt_debug)
    {
        ...
        applog(LOG_NOTICE, "Generated target %s", htarget);
        free(htarget);
    }
    memcpy(dest_target, target, 32);
}

void set_target_with_multiplier(unsigned char *dest_target, double diff,double multiplier)
{
    unsigned char target[32];
    uint64_t *data64, h64;
    double d64, dcut64;
...
static void gen_stratum_work_btc(struct pool *pool, struct work *work)
{
...
/*PATTERN_TEST_BEGIN(...)*/
	//pdata:010000009500c43a25c624520b5100adf82cb9f9da72fd2447a496bc600b0000000000006cd862370395dedf1da2841ccda0fc489e3039de5f1ccddef0e834991a65600ea6c8cb4db3936a1ae3143991
	//hex2bin(work->data, "010000009500c43a25c624520b5100adf82cb9f9da72fd2447a496bc600b0000000000006cd862370395dedf1da2841ccda0fc489e3039de5f1ccddef0e834991a65600ea6c8cb4db3936a1ae3143991",80); 
	//be32enc_vect(tmp_data,(const uint32_t*)work->data,20);
	//memcpy(work->data,tmp_data,80);
	...
	midstate_btc((uint32_t *)(work->midstate), (uint32_t *) (work->data));
	set_target_with_multiplier(work->target, work->sdiff,1);
...
{
	  if(opt_coin_type == COIN_BTC){
		gen_stratum_work_btc(pool,work);
	}else{
        quit (1, "not support algorithm");
	}
}
...
struct work *get_work(struct thr_info *thr, const int thr_id)
{
    ...
    /*applog(LOG_ERR, "cgpu->drv->max_diff:%8.4f,work->work_difficulty:%8.4f,cgpu->drv->min_diff:%8.4f", cgpu->drv->max_diff,work->work_difficulty,cgpu->drv->min_diff);*/
    return work;
}
...
void rebuild_nonce_btc(struct work *work, uint32_t nonce)
{
...
#endif
    mutex_lock(&stats_lock);
    total_diff1 += (work->device_diff<1.0?1.0:work->device_diff);
    thr->cgpu->diff1 += (work->device_diff<1.0?1:work->device_diff);
    work->pool->diff1 += (work->device_diff<1.0?1:work->device_diff);
...
bool submit_nonce_direct(struct thr_info *thr, struct work *work, uint32_t nonce)
{
    ...
    if(opt_coin_type == COIN_BTC){
		work_nonce = (uint32_t*) (work->data + 76);
	}else{
        quit (1, "not support algorithm");
    }
...
void *miner_thread(void *userdata)
{
    ...
    thread_reportout(mythr);
    if (!drv->thread_init(mythr))//bitmainA3_drv
    {
...
static json_t* test_curl(struct pool *pool,char* t_req)
{
    json_t *val,*jsoin_id,*result;
    CURL *curl;
    char *tmp = pool->rpc_url+11;
    char tmp_url[50];
...
# ifdef __linux
	...
	/*applog(LOG_WARNING, "start curl test ...");
	int x=1;
	char t_req[200];
	while(1){
		sprintf(t_req,"{\"id\":%d,\"jsonrpc\":\"2.0\",\"method\":\"getChallengeNumber\",\"params\":[\"0x7d41e3b6cb35b5301b996792572761d2a410a9da\"]}",x++);
		test_curl(currentpool,t_req);
		

		sprintf(t_req,"{\"id\":%d,\"jsonrpc\":\"2.0\",\"method\":\"getPoolEthAddress\",\"params\":[\"0x7d41e3b6cb35b5301b996792572761d2a410a9da\"]}",x++);
		test_curl(currentpool,t_req);
		

		sprintf(t_req,"{\"id\":%d,\"jsonrpc\":\"2.0\",\"method\":\"getMinimumShareTarget\",\"params\":[\"0x7d41e3b6cb35b5301b996792572761d2a410a9da\"]}",x++);
		test_curl(currentpool,t_req);
		

		sprintf(t_req,"{\"id\":%d,\"jsonrpc\":\"2.0\",\"method\":\"getMinimumShareDifficulty\",\"params\":[\"0x7d41e3b6cb35b5301b996792572761d2a410a9da\"]}",x++);
		test_curl(currentpool,t_req);
		sleep(10);
	}*/
...
begin_bench:
    ...
    retry:
        if (pool->has_stratum)
        {
            ...
            gen_stratum_work(pool, work);
            //applog(LOG_NOTICE, "stage_work stratum work");
